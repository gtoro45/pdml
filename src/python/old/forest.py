...
# CONN
def test_modular_conn(exclude_incomplete=True):
    # specify the paths
    conn_paths = [
    ...
    path7 = conn_paths[7]       # Benign
    path8 = conn_paths[8]       # Benign
    path9 = conn_paths[9]       # Benign
    path10 = conn_paths[10]     # Benign
    path11 = conn_paths[11]     # Benign
    
    path12 = conn_paths[12]     # Malignant
    path13 = conn_paths[13]     # Malignant
    path14 = conn_paths[14]     # Malignant 
    path15 = conn_paths[15]     # Malignant
    path16 = conn_paths[16]     # Malignant
    
    X7, encoder = encode_training_data(path7, fit_encoder=True, exclude_incomplete=exclude_incomplete)
    ...
    X = pd.concat([X7, X8, X9, X10, X11, X12, X13, X14, X15, X16], ignore_index=True)
    # X = X1
    ...
    iso_forest = IsolationForest(
        n_estimators=100,
        contamination=0.18675,  # mixing benign and attack sets
        random_state=42,
        n_jobs=-1
    )
    ...
    # encode test data
    X_train7, _ = encode_training_data(conn_paths[7], encoder=encoder, fit_encoder=False)
    X_train8, _ = encode_training_data(conn_paths[8], encoder=encoder, fit_encoder=False)
    X_train9, _ = encode_training_data(conn_paths[9], encoder=encoder, fit_encoder=False)
    X_train10, _ = encode_training_data(conn_paths[10], encoder=encoder, fit_encoder=False)
    X_train11, _ = encode_training_data(conn_paths[11], encoder=encoder, fit_encoder=False)
    
    X_attack12, _ = encode_training_data(conn_paths[12], encoder=encoder, fit_encoder=False)
    X_attack13, _ = encode_training_data(conn_paths[13], encoder=encoder, fit_encoder=False)
    X_attack14, _ = encode_training_data(conn_paths[14], encoder=encoder, fit_encoder=False)
    X_attack15, _ = encode_training_data(conn_paths[15], encoder=encoder, fit_encoder=False)
    X_attack16, _ = encode_training_data(conn_paths[16], encoder=encoder, fit_encoder=False)

    # run the model on test data
    y_train7 = iso_forest.predict(X_train7) # Changed 1 to 7
    y_train8 = iso_forest.predict(X_train8) # Changed 2 to 8
    y_train9 = iso_forest.predict(X_train9) # Changed 3 to 9
    y_train10 = iso_forest.predict(X_train10) # Changed 4 to 10
    y_train11 = iso_forest.predict(X_train11) # Added y_train11

    y_attack12 = iso_forest.predict(X_attack12) # Changed 1 to 12
    y_attack13 = iso_forest.predict(X_attack13) # Changed 2 to 13
    y_attack14 = iso_forest.predict(X_attack14) # Changed 3 to 14
    y_attack15 = iso_forest.predict(X_attack15) # Changed 4 to 15
    y_attack16 = iso_forest.predict(X_attack16) # Changed 5 to 16
    ...
    # y_train7 (conn_paths[7])
    print("***********************************************************************")
    print(f"Test set: {conn_paths[7]}")
    false_positives = (y_train7 == -1).sum() # Changed y_train1 to y_train7
    total = len(y_train7)
    print(f"Total benign test samples: {total}")
    print(f"False positives: {false_positives}")
    print(f"False positive rate: {false_positives / total:.4%}\n")
    ...
    # y_train8 (conn_paths[8])
    print("***********************************************************************")
    print(f"Test set: {conn_paths[8]}")
    false_positives = (y_train8 == -1).sum() # Changed y_train2 to y_train8
    total = len(y_train8)
    print(f"Total benign test samples: {total}")
    print(f"False positives: {false_positives}")
    print(f"False positive rate: {false_positives / total:.4%}\n")
    ...
    # y_train9 (conn_paths[9]) - Mapped from original y_train3, which used conn_paths[12]
    print("***********************************************************************")
    print(f"Test set: {conn_paths[9]}") # Changed 12 to 9
    false_positives = (y_train9 == -1).sum() # Changed y_train3 to y_train9
    total = len(y_train9)
    print(f"Total benign test samples: {total}")
    print(f"False positives: {false_positives}")
    print(f"False positive rate: {false_positives / total:.4%}\n")
    ...
    # y_train10 (conn_paths[10]) - Mapped from original y_train4, which used conn_paths[13]
    print("***********************************************************************")
    print(f"Test set: {conn_paths[10]}") # Changed 13 to 10
    false_positives = (y_train10 == -1).sum() # Changed y_train4 to y_train10
    total = len(y_train10)
    print(f"Total benign test samples: {total}")
    print(f"False positives: {false_positives}")
    print(f"False positive rate: {false_positives / total:.4%}\n")
    ...
    # New: y_train11 (conn_paths[11])
    print("***********************************************************************")
    print(f"Test set: {conn_paths[11]}") 
    false_positives = (y_train11 == -1).sum() 
    total = len(y_train11)
    print(f"Total benign test samples: {total}")
    print(f"False positives: {false_positives}")
    print(f"False positive rate: {false_positives / total:.4%}\n")
    ...
    # y_attack12 (conn_paths[12]) - Mapped from original y_attack1, which used conn_paths[17]
    print("***********************************************************************")
    print(f"Test set: {conn_paths[12]}") # Changed 17 to 12
    flagged = (y_attack12 == -1).sum() # Changed y_attack1 to y_attack12
    total = len(y_attack12)
    print(f"Total attack samples: {total}")
    print(f"Detected anomalies: {flagged}")
    print(f"Detection rate: {flagged / total:.4%}\n")
    print("***********************************************************************")

    # y_attack13 (conn_paths[13]) - Mapped from original y_attack2, which used conn_paths[18]
    print("***********************************************************************")
    print(f"Test set: {conn_paths[13]}") # Changed 18 to 13
    flagged = (y_attack13 == -1).sum() # Changed y_attack2 to y_attack13
    total = len(y_attack13)
    print(f"Total attack samples: {total}")
    print(f"Detected anomalies: {flagged}")
    print(f"Detection rate: {flagged / total:.4%}\n")
    print("***********************************************************************")

    # y_attack14 (conn_paths[14]) - Mapped from original y_attack3, which used conn_paths[19]
    print("***********************************************************************")
    print(f"Test set: {conn_paths[14]}") # Changed 19 to 14
    flagged = (y_attack14 == -1).sum() # Changed y_attack3 to y_attack14
    total = len(y_attack14)
    print(f"Total attack samples: {total}")
    print(f"Detected anomalies: {flagged}")
    print(f"Detection rate: {flagged / total:.4%}\n")
    print("***********************************************************************")

    # y_attack15 (conn_paths[15]) - Mapped from original y_attack4, which used conn_paths[20]
    print("***********************************************************************")
    print(f"Test set: {conn_paths[15]}") # Changed 20 to 15
    flagged = (y_attack15 == -1).sum() # Changed y_attack4 to y_attack15
    total = len(y_attack15)
    print(f"Total attack samples: {total}")
    print(f"Detected anomalies: {flagged}")
    print(f"Detection rate: {flagged / total:.4%}\n")
    print("***********************************************************************")

    # y_attack16 (conn_paths[16]) - Mapped from original y_attack5, which used conn_paths[21]
    print("***********************************************************************")
    print(f"Test set: {conn_paths[16]}") # Changed 21 to 16
    flagged = (y_attack16 == -1).sum() # Changed y_attack5 to y_attack16
    total = len(y_attack16)
    print(f"Total attack samples: {total}")
    print(f"Detected anomalies: {flagged}")
    print(f"Detection rate: {flagged / total:.4%}\n")
    print("***********************************************************************")
...
def test_modular_conn_rf(exclude_incomplete=True):
    # specify the paths
    conn_paths = [
        "../../train_test_data/benign_1_min/node-1-child-3/csv files/node1_conn.csv",           # [0]
        "../../train_test_data/benign_1_min/node-2-child-4/csv files/node2_conn.csv",           # [1]
    ...
    # TRAINING: benign vs malignant sets
    benign_paths = [7, 8, 9, 10, 11]       # NEW 10-min benign
    mal_paths    = [12, 13, 14, 15, 16]    # NEW malignant attacks


    # ---- Encode benign ----
    X_benign = []
    X0, encoder = encode_training_data(conn_paths[benign_paths[0]],
                                       fit_encoder=True,
                                       exclude_incomplete=exclude_incomplete)
    X_benign.append(X0)

    for idx in benign_paths[1:]:
        Xi, _ = encode_training_data(conn_paths[idx],
                                     encoder=encoder,
                                     fit_encoder=False,
                                     exclude_incomplete=exclude_incomplete)
        X_benign.append(Xi)
    ...
    X_benign = pd.concat(X_benign, ignore_index=True)
    X_mal = pd.concat(X_mal, ignore_index=True)
    ...
    X_train = pd.concat([X_benign, X_mal], ignore_index=True)
    y_train = np.concatenate([y_benign, y_mal])

    print(f"Training dataset size = {len(X_train)} ({len(X_benign)} benign, {len(X_mal)} malignant)")
    ...
    # Testing Phase
    # ======================================
    def test_set(path, is_malicious):
        X_test, _ = encode_training_data(path, encoder=encoder, fit_encoder=False)
        ...
        if is_malicious:
            ...
            total = len(preds)
            print("***********************************************************************")
            print(f"ATTACK TEST SET: {path}")
            print(f"Total attack samples: {total}")
            print(f"Detected anomalies:  {detected}")
            print(f"Detection rate:      {detected/total:.4%}")
            print("***********************************************************************\n")
        else:
            fp = (preds == 1).sum()
            total = len(preds)
            print("***********************************************************************")
            print(f"BENIGN TEST SET: {path}")
            print(f"Total benign samples: {total}")
            print(f"False positives:       {fp}")
            print(f"False positive rate:   {fp/total:.4%}")
            print("***********************************************************************\n")

    # ---- Test benign sets ----
    for idx in benign_paths:
        test_set(conn_paths[idx], is_malicious=False)

    # ---- Test malignant sets ----
    for idx in mal_paths:
        test_set(conn_paths[idx], is_malicious=True)

def test_modular_conn_xgb(exclude_incomplete=True):
    # specify the paths
    conn_paths = [
        ...
        "../../train_test_data/benign_sim/csv/conn.csv",                                      
        "../../train_test_data/ddos_sim/csv/conn_malignant1.csv",                              
    ...
    # TRAINING SETS
    benign_paths = [7, 8, 9, 10, 11]    # NEW benign
    mal_paths    = [12, 13, 14, 15, 16] # NEW malicious
...
#     if "attack" in path:
#         malignant_sizes.append(length)
#     else:
#         benign_sizes.append(length)
#     print(f"{path} size = {length}")

# total_rows = sum(benign_sizes) + sum(malignant_sizes)
# contamination = sum(malignant_sizes) / total_rows
# print(f"Contamination = {contamination:.5f}")
# print()

test_modular_conn_rf(exclude_incomplete=False)
test_modular_conn_xgb(exclude_incomplete=False)

print()
print("==========================================================")
print()

test_modular_conn_rf(exclude_incomplete=True)
test_modular_conn_xgb(exclude_incomplete=True)
